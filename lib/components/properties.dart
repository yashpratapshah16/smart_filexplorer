...
class Properties {
  static void getProperties(String path, void Function(Map) onUpdate) async {
    ...
    try {
      ...
      if (await entity.exists()) {
        ...
        properties["Name"] = path.split(Platform.pathSeparator).last;
        ...
        properties["Last Modified"] = _formatDate(await _getLastModified(entity));
        onUpdate(Map.from(properties));
  ...
  static Future<void> _calculateWithProgress(
    ...
    void updateContains() {
      props["Contains"] = "$fileCount Files, $folderCount Folders (calculating...)";
  ...
  static Future<int> _getFolderSize(Directory directory, [Function(int)? onProgress]) async {
    int size = 0;
  ...
  static Future<int> _getFileCount(Directory dir, {Function(int)? onProgress}) async {
    int count = 0;
    await for (var entity in dir.list(recursive: false)) {
      if (entity is File) {
        ...
        count += await _getFileCount(Directory(entity.path), onProgress: (subCount) {
  ...
  static Future<int> _getFolderCount(Directory directory, {Function(int)? onProgress}) async {
    int count = 1;
    onProgress?.call(count);
    await for (FileSystemEntity entity in directory.list(recursive: false)) {
      if (entity is Directory) {
        count += await _getFolderCount(Directory(entity.path), onProgress: (subCount) {
          onProgress?.call(count + subCount);
        });
        onProgress?.call(count);
      }
  ...
  static String _formatDate(DateTime date) => DateFormat.yMEd().add_jms().format(date);
}
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            _properties["Type"]!="File"?_buildRow("Contains", _properties["Contains"]):Text(""),
            ...
            if (_properties["Error"]?.isNotEmpty == true)
              Text(_properties["Error"], style: const TextStyle(color: Colors.red)),
          ],
        ),
      ),
      actions: [
